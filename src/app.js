...
(function () {
  ...
  function ifaceType(n) {
    ...
    // VLAN sub-interface (e.g. bond0.10, eth0.100, enp3s0.50)
    if (/^[^.]+\.\d+$/.test(n) && !/^veth|^tap|^fw|^docker|^br-|^virbr/.test(n)) return 'vlan';
    // Bonding (bond0, bond1)
    if (/^bond\d/.test(n)) return 'bond';
    // Bridge (vmbr0, br-xxx, virbr0)
    if (/^(vmbr|br-|virbr)/.test(n)) return 'bridge';
    // Firewall/Proxy (Proxmox: fwpr, fwn, fwln, fwbr, fwp, fwt)
    ...
    // Physical ethernet (en*, eth*)
    if (/^(eth|enp|eno|ens|enx)/.test(n)) return 'ethernet';
    return 'ethernet';
  }
  var typeLabels = {
    ethernet: '物理网卡', bond: '绑定接口', vlan: 'VLAN 子接口',
    bridge: '网桥', wireless: '无线', firewall: '防火墙接口',
    tap: 'TAP 接口', veth: '虚拟以太网', virtual: '虚拟接口', loopback: '回环',
  };
  ...
  function ensureHistory(name) {
    if (!state.history[name]) {
      state.history[name] = {
        ...
        _lastMinuteBucket: null, _lastHourlyBucket: null, _lastDailyBucket: null,
  ...
  function fetchIfaceInfo(names) {
    ...
    if (typeof cockpit !== 'undefined') {
        ...
        .then(function (output) {
          ...
          for (var i = 0; i < lines.length; i++) {
            ...
            if (parts.length >= 7) {
              ...
              if (inUse) {
                // Find the device name for connected AP
      ...
      // Also try iw for interfaces not managed by NM
  ...
  function updateState(newIfaces) {
    ...
    for (var i = 0; i < newIfaces.length; i++) {
      ...
      // 在线判断：有流量时标记活跃时间，30秒内有过流量视为在线
      if (iface.speed > 0) {
        state.lastActive[iface.name] = now;
      }
      // 有IP地址的非回环接口也视为在线（可能是新发现的活跃接口）
  ...
  function getFiltered() {
    ...
    if (state.filters.status.size > 0) list = list.filter(function (i) { return state.filters.status.has(i.up ? '活跃' : '离线'); });
    if (state.filters.name.size > 0) list = list.filter(function (i) { return state.filters.name.has(i.name); });
    if (state.filters.type.size > 0) list = list.filter(function (i) { return state.filters.type.has(typeLabels[i.type]); });
  ...
  function drawLineChart(canvas, datasets, opts) {
    ...
    if (allX.length < 2) {
      ctx.fillStyle = 'var(--text-muted)'; ctx.font = '13px sans-serif'; ctx.textAlign = 'center';
  ...
  function renderStats() {
    ...
    document.getElementById('interface-count').textContent = ifs.length + ' 接口';
  }
  ...
  function renderChart() {
    ...
    state.chartDatasets = [
      { data: txData, color: '#f59e0b', fill: 'rgba(245,158,11,0.08)', label: '发送' },
      { data: rxData, color: '#3b82f6', fill: 'rgba(59,130,246,0.08)', label: '接收' },
    ];
  ...
  function renderTable() {
    ...
    for (var ri = 0; ri < filtered.length; ri++) {
      ...
      html += '<tr data-name="' + iface.name + '">'
        + '<td><div class="td-status"><span class="status-dot ' + (iface.up ? 'up' : 'down') + '"></span>' + (iface.up ? '活跃' : '离线') + '</div></td>'
        + '<td class="td-name">' + iface.name + '</td>'
        + '<td><span class="type-tag ' + iface.type + '">' + typeLabels[iface.type] + '</span></td>'
  ...
  function getUniqueValues(field) {
    ...
    for (var i = 0; i < all.length; i++) {
      var val;
      if (field === 'status') val = all[i].up ? '活跃' : '离线';
      else if (field === 'name') val = all[i].name;
      else if (field === 'type') val = typeLabels[all[i].type];
      else continue;
  ...
  function buildFilterDropdown(field, dropdown) {
    ...
    var html = '<div class="filter-search"><input type="text" placeholder="搜索..."></div>'
      + '<div class="filter-actions"><button class="btn-sel-all">全选</button><button class="btn-sel-none">清除</button></div><div class="filter-list">';
    ...
    html += '</div><div class="filter-footer"><button class="btn-clear">清除筛选</button><button class="btn-apply">确定</button></div>';
  ...
  function openDetail(name) {
    ...
    document.getElementById('modal-title').textContent = iface.name + ' - 接口详情';
    var basicRows = [
      ['接口名称', iface.name], ['类型', typeLabels[iface.type]],
      ['MAC 地址', iface.mac || '-'], ['IPv4 地址', iface.ip || '-'],
      ['IPv6 地址', iface.ipv6 || '-'],
    ];
    if (iface.type === 'wireless' && iface.wifi) {
      if (iface.wifi.ssid) basicRows.push(['连接网络', iface.wifi.ssid]);
      basicRows.push(['信号强度', (iface.wifi.signal || 0) + '%']);
      if (iface.wifi.bars) basicRows.push(['信号质量', iface.wifi.bars]);
      if (iface.wifi.band) basicRows.push(['频段', iface.wifi.band]);
      if (iface.wifi.chan) basicRows.push(['信道', iface.wifi.chan]);
      if (iface.wifi.security) basicRows.push(['加密方式', iface.wifi.security]);
      if (iface.wifi.rate) basicRows.push(['无线速率', iface.wifi.rate + ' Mbit/s']);
      if (iface.wifi.rxRate) basicRows.push(['接收速率', iface.wifi.rxRate]);
      if (iface.wifi.txRate) basicRows.push(['发送速率', iface.wifi.txRate]);
    } else {
      basicRows.push(['链接速率', fmtSpeed(iface.linkSpeed)]);
    }
    basicRows.push(['状态', iface.up ? '🟢 活跃' : '🔴 离线']);
    document.getElementById('detail-basic').innerHTML = mkRows(basicRows);
    document.getElementById('detail-stats').innerHTML = mkRows([
      ['总发送流量', fmt(iface.txBytes)], ['总接收流量', fmt(iface.rxBytes)],
      ['总流量', fmt(iface.totalBytes)], ['发送数据包', iface.txPackets.toLocaleString()],
      ['接收数据包', iface.rxPackets.toLocaleString()],
      ['当前发送速率', fmtS(iface.txSpeed)], ['当前接收速率', fmtS(iface.rxSpeed)],
    ]);
    document.getElementById('detail-errors').innerHTML = [
      mkErr('发送错误', iface.txErrors), mkErr('接收错误', iface.rxErrors),
      mkErr('发送丢包', iface.txDropped), mkErr('接收丢包', iface.rxDropped),
    ].join('');
    // WiFi section
  ...
  function renderWifiInfo(iface) {
    ...
    if (scanList.length === 0) {
      scanEl.innerHTML = '<div class="wifi-empty">暂无扫描数据</div>';
      return;
    }
    // Sort by signal descending, put connected on top
    scanList.sort(function (a, b) { return (b.inUse ? 1000 : 0) + b.signal - (a.inUse ? 1000 : 0) - a.signal; });
    var html = '<table class="wifi-table"><thead><tr><th></th><th>网络名称</th><th>信号</th><th>频段</th><th>信道</th><th>加密</th></tr></thead><tbody>';
  ...
  function mkRows(pairs) {
    return pairs.map(function (p) { return '<div class="detail-row"><span class="label">' + p[0] + '</span><span class="value">' + p[1] + '</span></div>'; }).join('');
  }
  function mkErr(label, val) {
    var cls = val === 0 ? 'zero' : val < 10 ? 'warning' : 'danger';
    return '<div class="error-stat-item"><div class="error-label">' + label + '</div><div class="error-value ' + cls + '">' + val + '</div></div>';
  }

  function renderDetailCharts(name) {
    var cd = getChartData(name, state.detailTimeRange);
    state.detailChartDatasets = [
      { data: cd.tx, color: '#f59e0b', fill: 'rgba(245,158,11,0.08)', label: '发送' },
      { data: cd.rx, color: '#3b82f6', fill: 'rgba(59,130,246,0.08)', label: '接收' },
    ];
    ...
    state.detailSpeedDatasets = [
      { data: cd60.tx, color: '#f59e0b', fill: 'rgba(245,158,11,0.08)', label: '发送' },
      { data: cd60.rx, color: '#3b82f6', fill: 'rgba(59,130,246,0.08)', label: '接收' },
    ];
